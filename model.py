...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        return checkpoint(partial(attn_fn), x, use_reentrant=False)
...
class GPT(nn.Module):
    ...
    @torch.no_grad()
    def generate(self, idx, max_new_tokens, temperature=1.0, top_k=None, eos_token_ids=None):
        stop_after = int(0.8 * max_new_tokens)
        ...
        for step in range(max_new_tokens):
            ...
            if (step + 1) >= stop_after and next_id in eos_token_ids:
                break
        return idx